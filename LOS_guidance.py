...
def call_distance(P1: np.ndarray, P2: np.ndarray, DMM: bool = False):
    """Calculate the distance between two points (DEG or DMM format).
    ...
    # Convert DMM format to DEG format if needed
    if DMM:
        P1 = DMM_to_DEG(P1)
        P2 = DMM_to_DEG(P2)
...
def LOS_latlon(position: np.ndarray,
    ...
    lon_proj = (los_s) * cos(alpha) * deg_meter_lon + previous_waypoint[
        1]  # lon projection in degrees + lon of previousWP
    lat_proj = (los_s) * sin(alpha) * deg_meter_lat + previous_waypoint[
        0]  # lat projection in degrees + lat of previous WP
    ...
    x = np.sqrt(((current_waypoint[0] - previous_waypoint[0]) * meter_deg_lat) ** 2 + (
                (current_waypoint[1] - previous_waypoint[1]) * meter_deg_lon) ** 2)
    if los_s > x:
        los_lon = (lon_proj - los_delta * deg_meter_lon * cos(alpha) - position[1]) * meter_deg_lon
        los_lat = (lat_proj - los_delta * deg_meter_lat * sin(alpha) - position[0]) * meter_deg_lat
